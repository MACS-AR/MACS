...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Catuserbot"):
            buttons = [
                (
                    ...
                    Button.url("DEV", "https://t.me/MACS37"),
                )
            ]
            ...
            result = builder.article(
                title="MACS Help™",
                description="**▾∮ قائمة التعليمات الخاصة بــ ماكس **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/MACS37"),
                Button.url(
                    "المطور",
                    "t.me/hamo171002",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل ماكس من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝙅𝙈𝙏𝙃𝙊𝙉 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/MACS37)",
